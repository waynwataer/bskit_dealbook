...
### 지원 파일 형식 (v13.3 — PDF 분석 지원)

가장 관련 높은 파일 **1개**를 형식에 따라 자동으로 최적 경로로 분석합니다.

- **PDF·이미지**: `GEMINI_API_KEY`가 있으면 Gemini에 파일을 통째로 넘겨 분석합니다.
  표·차트가 많은 부동산 리포트에 강합니다(별도 텍스트 추출 없이 레이아웃까지 이해).
  용량 약 14MB 초과 시에는 링크만 안내합니다.
- **Google 문서·스프레드시트·프레젠테이션 / .txt·.csv**: 텍스트를 추출해 GPT가 분석합니다.
- **Word(.docx) 등 기타**: 아직 자동 분석 미지원 — 파일명·링크만 표시됩니다.

원하는 문서가 1순위로 잡히도록, 질문에 **파일명 일부**(예: "262Q Korea Office Market
Report 요약해줘")를 넣으면 파일명 매칭으로 그 문서를 우선 분석합니다.

> PDF 분석에는 `GEMINI_API_KEY`가 필요합니다. Gemini 키가 없으면 PDF는 분석되지 않고
> 링크만 표시됩니다.
